                    ...
                    <div class="col-md-12">
                        <div class="card-body box-profile">
                            <div class="text-center">
                                <img class="profile-user-img img-fluid img-circle" style="border: 3px solid #5B86E5 !important; padding: 5px !important;" src="{{ fileExist(['url' => auth()->user()->image, 'type' => 'profile']) }}" alt="{{ auth()->user()->name }}">
                            </div>
                            <h3 class="profile-username text-center">{{ auth()->user()->name }}</h3>
                            <p class="text-muted text-center">{!! '<span class="badge badge-success mr-1">' .
                                implode(
                                    '</span><span class="badge badge-success mr-1">',
                                    auth()->user()->user_roles->pluck('role_details')->pluck('name')->toArray(),
                                ) .
                                '</span>' !!}</p>
                        </div>
                    </div>
