			...
			<div class="col-lg-12">
				<div class="card">
					<div class="card-header text-right">  
						...
						<a href="{{route('admin.member-management.member-info.list') }}" class="btn btn-sm btn-info"><i class="fas fa-list"></i> Member List</a>
					</div>
					<div class="card-body">
						<form id="submitForm">
							@csrf
								...
								<div class="col-sm-8">
									<div class="form-group text-right">
										...
										<button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
											<a href="{{route('admin.member-management.member-info.list') }}">Back</a>
										</button>
									</div>
								</div>
							</div>
	...
	$(document).ready(function(){
		$('#submitForm').validate({
			...
			errorPlacement: function(error, element){
				if(element.hasClass("status")){error.insertAfter(element.next()); }
			...
			submitHandler: function (form) {
				...
				$.ajax({
					url : "{{ isset($editData) ? route('admin.member-management.member-info.update',$editData) : route('admin.member-management.member-info.store') }}",
					data : formInfo,
					type : "POST",
					processData: false,
					contentType: false,
		...
		jQuery.validator.addClassRules({
			...
			'email' : {
				...
				email : true,
				remote: {
					url: "{{route('admin.member-management.member-info.duplicate-email-check')}}",
					type: "GET",
					data: {
			...
			'mobile_no' : {
				...
				// maxlength:5,
				remote: {
					url: "{{route('admin.member-management.member-info.duplicate-mobile_no-check')}}",
					type: "GET",
					data: {
			...
			'password' : {
				required : function(){return $('#change_password').prop('checked');},
			},
			'confirm_password' : {
				required : function(){return $('#change_password').prop('checked');},
				equalTo : '#password',
